...
class YuvAwbMapperTest {
    @Test
    fun systemAutoEquivalentGainsResolveToIdentity() {
        val gains = floatArrayOf(2.0f, 1.0f, 1.0f, 1.6f)
        assertArrayEquals(
            floatArrayOf(1f, 1f, 1f),
            YuvAwbMapper.resolve(gains, gains),
            1.0e-6f
        )
    }

    @Test
    fun liveTargetIsMappedRelativeToCameraAwb() {
        val result = YuvAwbMapper.resolve(
            baseCameraGains = floatArrayOf(2.0f, 1.0f, 1.0f, 1.5f),
            targetGains = floatArrayOf(1.5f, 1.0f, 1.0f, 2.0f)
        )
    ...
    fun invalidOrExtremeInputsFailClosedAndClamp() {
        assertArrayEquals(
            floatArrayOf(1f, 1f, 1f),
            YuvAwbMapper.resolve(null, null),
            1.0e-6f
        )
        val clamped = YuvAwbMapper.resolve(
