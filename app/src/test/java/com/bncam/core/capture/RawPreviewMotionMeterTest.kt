...
class RawPreviewMotionMeterTest {
    ...
    fun `first frame warms temporal baseline`() {
        ...
        val first = meter.observeNv21(texturedNv21(160, 120), 160, 120, 4080, 3060, 1_000_000_000L)
        assertFalse(first.ready)
    }

    @Test
    fun `translated textured frame produces motion evidence`() {
        val meter = RawPreviewMotionMeter()
        meter.observeNv21(texturedNv21(160, 120), 160, 120, 4080, 3060, 1_000_000_000L)
        val moved = meter.observeNv21(texturedNv21(160, 120, shiftX = 1), 160, 120, 4080, 3060, 1_033_333_333L)
    ...
    fun `reset requires a new temporal pair`() {
        val meter = RawPreviewMotionMeter()
        meter.observeNv21(texturedNv21(160, 120), 160, 120, 4080, 3060, 1_000_000_000L)
        meter.observeNv21(texturedNv21(160, 120, shiftX = 1), 160, 120, 4080, 3060, 1_033_333_333L)
        meter.reset()
        val afterReset = meter.observeNv21(texturedNv21(160, 120), 160, 120, 4080, 3060, 2_000_000_000L)
    ...
    fun `global brightness change is not treated as strong motion`() {
        ...
        val result = meter.observeNv21(brighter, 160, 120, 4080, 3060, 1_033_333_333L)
