...
struct Result { Pixel pixel; float hf=0, mid=0, structure=0; };
...
Result filter(int x, int y, Model model, Pixel postChroma, Read read, Shape shape) {
    ...
    for(int band=0;band<2;++band) {
        ...
        float a=physical*(1.f-bandStructure)*(band==0?0.28f:0.16f);
        for(int dir=0;dir<4;++dir) {
    ...
    return {out,authority[0],authority[1],structure};
}
} // namespace bncam::luma
