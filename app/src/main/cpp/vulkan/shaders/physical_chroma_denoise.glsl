...
float physicalChromaShape(ivec2 p) {
    ...
    vec2 uv=clamp((vec2(p)+0.5)/vec2(pc.frameWidth,pc.frameHeight),vec2(0),vec2(1));
    ivec2 t=min(ivec2(uv*vec2(cols,rows)),ivec2(cols,rows)-1);
    return cloudCorrectionMap[uint(t.y)*cols+uint(t.x)].x;
}
