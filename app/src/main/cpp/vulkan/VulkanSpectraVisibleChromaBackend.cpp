...
struct alignas(16) VisibleChromaPushConstants {
    std::uint32_t width = 0;
    std::uint32_t height = 0;
    float chromaStrength = 0.0f;
    float padding = 0.0f;
};
static_assert(sizeof(VisibleChromaPushConstants) == 16u,
        "Visible chroma push constant layout must match SPIR-V");
...
bool VulkanSpectraVisibleChromaBackend::productionKernelConnected() const noexcept {
    return !getIspChromaDenoiseSpirv().empty();
}

bool VulkanSpectraVisibleChromaBackend::pipelineInitialized() const noexcept {
    std::lock_guard<std::mutex> lock(mutex_);
    return initialized_;
}

void VulkanSpectraVisibleChromaBackend::destroyPersistentBuffersLocked() noexcept {
#if BNCAM_VMA_HEADER_AVAILABLE
...
bool VulkanSpectraVisibleChromaBackend::ensurePersistentBuffersLocked(
        VmaAllocator allocator,
...
bool VulkanSpectraVisibleChromaBackend::initializeLocked(
        VkDevice device,
        std::string& failureReason
) noexcept {
    ...
    const auto& spirv = getIspChromaDenoiseSpirv();
    if (spirv.empty()) {
        failureReason = "SPECTRA_VISIBLE_CHROMA_SPIRV_UNAVAILABLE";
        return false;
    }
    ...
    pushRange.offset = 0u;
    pushRange.size = sizeof(VisibleChromaPushConstants);
    ...
    if (vkCreateShaderModule(device, &shaderInfo, nullptr, &shaderModule_) != VK_SUCCESS) {
        failureReason = "vkCreateShaderModule_spectra_visible_chroma_failed";
        destroyLocked(device);
        return false;
    }
    ...
    if (VulkanPipelineCacheRegistry::createComputePipelines(device, 1u, &pipelineInfo, nullptr, &pipeline_
        ...
        failureReason = "vkCreateComputePipelines_spectra_visible_chroma_failed";
        destroyLocked(device);
        return false;
    }
    ...
    if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &descriptorPool_) != VK_SUCCESS) {
        failureReason = "vkCreateDescriptorPool_spectra_visible_chroma_failed";
        destroyLocked(device);
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(device, &allocateInfo, &descriptorSet_) != VK_SUCCESS) {
        failureReason = "vkAllocateDescriptorSets_spectra_visible_chroma_failed";
        destroyLocked(device);
        return false;
    }
...
void VulkanSpectraVisibleChromaBackend::destroyLocked(VkDevice device) noexcept {
    destroyPersistentBuffersLocked();
    if (device != VK_NULL_HANDLE) {
...
void VulkanSpectraVisibleChromaBackend::destroy(VkDevice device) noexcept {
    std::lock_guard<std::mutex> lock(mutex_);
    destroyLocked(device);
}

SpectraVisibleChromaExecutionResult VulkanSpectraVisibleChromaBackend::execute(
    ...
    if (request.rgbData == nullptr || request.width == 0u || request.height == 0u ||
        ...
        !std::isfinite(request.chromaStrength)) {
        result.failureReason = "INVALID_SPECTRA_VISIBLE_CHROMA_INPUT";
        result.status = "FAILED";
        result.totalMs = elapsedMs(totalStarted);
        return result;
    }
    ...
    if (pixels > static_cast<std::uint64_t>(
        ...
        result.failureReason = "SPECTRA_VISIBLE_CHROMA_FRAME_TOO_LARGE";
        result.status = "FAILED";
        result.totalMs = elapsedMs(totalStarted);
        return result;
    }
    ...
    VisibleChromaPushConstants push{};
    push.width = request.width;
    push.height = request.height;
    push.chromaStrength = std::clamp(request.chromaStrength, 0.0f, 1.0f);
    vkCmdPushConstants(
            commandBuffer,
            pipelineLayout_,
    ...
    if (vkQueueSubmit(computeQueue, 1u, &submit, fence) != VK_SUCCESS ||
        ...
        VulkanRuntime::instance().markGpuStalled("VisibleChromaBackend");
        result.failureReason = "VULKAN_SPECTRA_VISIBLE_CHROMA_SUBMISSION_OR_WAIT_FAILED";
    ...
    try {
        result.filteredRgba.resize(
                static_cast<std::size_t>(pixels) * static_cast<std::size_t>(kFloatsPerPixel)
        );
    } catch (...) {
        result.failureReason = "SPECTRA_VISIBLE_CHROMA_CPU_READBACK_ALLOCATION_FAILED";
    ...
    result.success = true;
    result.status = "SPECTRA_FP32_VISIBLE_CHROMA_CANDIDATE_READY";
    cleanup();
    result.totalMs = elapsedMs(totalStarted);
    return result;
#endif
