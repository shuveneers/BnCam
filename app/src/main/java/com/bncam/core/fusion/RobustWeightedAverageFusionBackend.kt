...
class RobustWeightedAverageFusionBackend : FusionBackend {
    override val id: String = "Robust Weighted Average"
    ...
    override fun fuse(
        ...
        if (acceptedSecondaries.isEmpty()) {
            return FusionResult(
                ...
                validOverlapPercentage = 100.0f,
                fusedPayload = referencePayload,
                fallbackReason = "All secondary frames rejected by alignment gate",
                processingTimeMs = System.currentTimeMillis() - startMs,
                peakMemoryUsageBytes = referencePayload.size.toLong()
            )
        }
        ...
        perFrameWeights[0] = 1.0f / numAccepted

        var motionRejectedCount = 0L
        var totalPixelsCount = 0L

        // Noise-aware weighted accumulation
        ...
        for (i in 0 until len step 2) {
            ...
            for (sIdx in acceptedSecondaries.indices) {
                ...
                val sigmaNoise = 45.0f

                // Huber / sigma-clipping penalty
                val w = (1.0f / (1.0f + (diff * diff) / (sigmaNoise * sigmaNoise))) * align.globalConfidence
