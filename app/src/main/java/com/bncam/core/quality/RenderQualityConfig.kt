...
data class RenderQualityConfig(
    ...
    companion object {
        ...
        private data class ColorMatrixCandidate(
            val source: String,
            val values: FloatArray,
            val construction: String
        )

        private data class ColorMatrixValidationResult(
            val valid: Boolean,
            val score: Float,
            val reason: String
        )

        private fun normalizeColorCorrectionMode(mode: String): String {
            val normalized = mode.trim().uppercase(Locale.US)
            return when (normalized) {
                "",
                "SAFE_IDENTITY_UNTIL_CALIBRATED",
                "CAMERA_METADATA_EXPERIMENTAL",
                "VALIDATED_METADATA" -> "AUTO_METADATA_VALIDATED"
                else -> normalized
            }
        }
        ...
        private fun invert3x3(m: FloatArray): FloatArray? {
            ...
            val det = m[0] * (m[4] * m[8] - m[5] * m[7]) -
                m[1] * (m[3] * m[8] - m[5] * m[6]) +
                m[2] * (m[3] * m[7] - m[4] * m[6])
            if (!det.isFinite() || kotlin.math.abs(det) < 0.00001f) return null
            val invDet = 1f / det
            return floatArrayOf(
        ...
        private fun colorMatrixSourcePriority(source: String): Int {
            // A validated per-frame COLOR_CORRECTION_TRANSFORM is already the camera device's
            // sensor-RGB -> linear-sRGB solution for the current capture state, so it is the
            // authoritative first choice. Static ForwardMatrix metadata is the calibrated
            // fallback; inverse ColorTransform remains last-resort reconstruction.
            return when {
                source.contains("COLOR_CORRECTION_TRANSFORM", ignoreCase = true) -> 0
                source.contains("SENSOR_FORWARD_MATRIX2", ignoreCase = true) -> 1
                source.contains("SENSOR_FORWARD_MATRIX1", ignoreCase = true) -> 2
                source.contains("SENSOR_COLOR_TRANSFORM2", ignoreCase = true) -> 3
                source.contains("SENSOR_COLOR_TRANSFORM1", ignoreCase = true) -> 4
                else -> 5
            }
        }
        ...
        private fun resolveColorCorrectionMatrix(
            ...
            val normalizedMode = normalizeColorCorrectionMode(mode)
            if (normalizedMode == "OFF" || normalizedMode == "DISABLED" || normalizedMode == "IDENTITY_ONLY") {
                return ColorCorrectionMatrix.identity(
                    source = "USER_DISABLED_IDENTITY",
                    note = "Color correction mode=$normalizedMode intentionally disables camera metadata matrix application."
                )
            }

            val candidates = mutableListOf<ColorMatrixCandidate>()
            ...
            characteristics.get(CameraCharacteristics.SENSOR_FORWARD_MATRIX2)?.let { transform ->
                addMatrixCandidate(
                    ...
                    values = multiply3x3(RawColorTransformEngine.xyzD50ToLinearSrgbMatrix(), colorSpaceTransformToArray(transform)),
                    construction = "camera_to_xyz_d50_forward_matrix_then_bradford_d65_then_linear_srgb"
                )
            }
            ...
            characteristics.get(CameraCharacteristics.SENSOR_COLOR_TRANSFORM2)?.let { transform ->
                val cameraToXyz = invert3x3(colorSpaceTransformToArray(transform))
                addMatrixCandidate(
                    target = candidates,
                    source = "inverse(CameraCharacteristics.SENSOR_COLOR_TRANSFORM2) -> sRGB",
                    values = cameraToXyz?.let { multiply3x3(xyzToSrgbD65Matrix(), it) },
                    construction = "inverse_xyz_to_camera_then_xyz_to_srgb"
                )
            }
            characteristics.get(CameraCharacteristics.SENSOR_COLOR_TRANSFORM1)?.let { transform ->
                ...
                addMatrixCandidate(
                    target = candidates,
                    source = "inverse(CameraCharacteristics.SENSOR_COLOR_TRANSFORM1) -> sRGB",
                    values = cameraToXyz?.let { multiply3x3(xyzToSrgbD65Matrix(), it) },
                    construction = "inverse_xyz_to_camera_then_xyz_to_srgb"
                )
            }
            ...
            val accepted = candidates.mapNotNull { candidate ->
                val validation = validateColorMatrix(candidate.values)
                if (validation.valid) {
                    candidate to validation
                } else {
                    rejected.add("${candidate.source} rejected: ${validation.reason}")
                    null
                }
            }

            val best = accepted.minWithOrNull(
                compareBy<Pair<ColorMatrixCandidate, ColorMatrixValidationResult>>(
                    { colorMatrixSourcePriority(it.first.source) },
                    { it.second.score }
                )
            )
            if (best != null) {
                val candidate = best.first
                val validation = best.second
            ...
            val note = if (rejected.isEmpty()) {
                "No usable camera color metadata matrix was present; controlled identity fallback used and flagged."
            } else {
                rejected.joinToString(" | ") + "; controlled identity fallback used and flagged."
            }
            return ColorCorrectionMatrix.identity(
        ...
        private fun validateColorMatrix(values: FloatArray): ColorMatrixValidationResult {
            if (values.size != 9) return ColorMatrixValidationResult(false, Float.MAX_VALUE, "expected 9 values, got ${values.size}")
            if (values.any { !it.isFinite() }) return ColorMatrixValidationResult(false, Float.MAX_VALUE, "non-finite matrix value")

            val rowSums = FloatArray(3) { row ->
                values[row * 3] + values[row * 3 + 1] + values[row * 3 + 2]
            }
            val rowAbs = FloatArray(3) { row ->
                kotlin.math.abs(values[row * 3]) + kotlin.math.abs(values[row * 3 + 1]) + kotlin.math.abs(values[row * 3 + 2])
            }
            if (rowSums.any { !it.isFinite() || kotlin.math.abs(it) < 0.0001f }) {
                return ColorMatrixValidationResult(false, Float.MAX_VALUE, "neutral row sum invalid ${rowSums.toList()}")
            }
            if (rowAbs.any { !it.isFinite() || it < 0.05f || it > 8.0f }) {
                return ColorMatrixValidationResult(false, Float.MAX_VALUE, "row gain outside safe range ${rowAbs.toList()}")
            }

            val det = values[0] * (values[4] * values[8] - values[5] * values[7]) -
                values[1] * (values[3] * values[8] - values[5] * values[6]) +
                values[2] * (values[3] * values[7] - values[4] * values[6])
            if (!det.isFinite() || kotlin.math.abs(det) < 0.001f || kotlin.math.abs(det) > 24.0f) {
                return ColorMatrixValidationResult(false, Float.MAX_VALUE, "determinant outside safe range ${String.format(Locale.US, "%.5f", det)}")
            }

            val meanRowSum = rowSums.average().toFloat()
            val neutralDeviation = rowSums.maxOf { kotlin.math.abs(it - meanRowSum) } / kotlin.math.max(0.0001f, kotlin.math.abs(meanRowSum))
            val negativeEnergy = values.filter { it < 0f }.sumOf { kotlin.math.abs(it).toDouble() }.toFloat()
            val maxAbs = values.maxOf { kotlin.math.abs(it) }
            val score = neutralDeviation + negativeEnergy * 0.08f + kotlin.math.abs(maxAbs - 1.0f) * 0.04f
            return ColorMatrixValidationResult(true, score, "valid")
        }
